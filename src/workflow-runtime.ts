...
export async function runArgusResearch(
	event: WorkflowEvent<ResearchRequest>,
	step: WorkflowStep,
	env: Env
): Promise<ArgusOutput> {
	const question = normalizeQuestion(event.payload.question);
	const trace = [`accepted question: ${question}`];
	const facets = await step.do('facet-question', async () => buildFacets(question, env));
	trace.push(`generated ${facets.length} facets`);
	const firstPass: SearchReport[] = [];
	for (const facet of facets) firstPass.push(await step.do(`search-${facet.id}`, async () => searchFacet(question, facet, env)));
	...
	for (const gap of firstPass.flatMap((report) => (report.gap ? [report.gap] : []))) {
		const facet = { id: `${gap.facetId}-follow-up`, label: `${gap.facet} follow-up`, query: gap.query, why: gap.reason };
		followUps.push(await step.do(`follow-up-${gap.facetId}`, async () => searchFacet(question, facet, env, 'follow-up')));
	}
	...
	const answer = await step.do('synthesize-evidence-board', async () => synthesize(graph, env));
